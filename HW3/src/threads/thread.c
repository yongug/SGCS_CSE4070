...
bool thread_compare (const struct list_elem *left, const struct list_elem *right, void *aux UNUSED);
...
bool
thread_compare (const struct list_elem *left, const struct list_elem *right, void *aux UNUSED)
{
  bool tf;
  ...
  tf = (thread_left->priority > thread_right->priority);
  return tf;
}
...
thread_yield (void) 
{
  ...
  old_level = intr_disable ();
  if (cur != idle_thread) 
    //proj3 list_push_back -> list_inset_ordered
...
thread_set_nice (int nice) 
{
	struct thread *cur = thread_current ();
  	cur->nice = nice;
  	cur->priority = PRI_MAX * FR - (cur->recent_cpu / 4) - (((nice * FR) * 2) / FR);
...
{
	return (100 * thread_current ()->recent_cpu) / FR;	
}
...
update_priority ()
{
  struct list_elem *e;
	...
	// 	t->priority = (PRI_MAX * FR - (t->recent_cpu / 4) - (t->nice * 2) * FR) / FR;
...
update_recent_cpu ()
{
  ...
  load_avg += (ready * FR) / 60;

	for (e = list_begin (&all_list); e != list_end (&all_list); e = list_next (e)) {
		t = list_entry (e, struct thread, allelem);
		if (t != idle_thread)
			t->recent_cpu = ((int64_t) ((int64_t) 2 * load_avg) * FR / (2 * load_avg + FR)) * t->recent_cpu / FR + t->nice * FR;
	}
}

